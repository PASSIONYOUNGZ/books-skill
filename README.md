...
Books Skill 让 agent 从“会概括一本书”升级为“能严肃整理一批书”。它不是把章节标题扩写成几段摘要，而是把抽取、精读、结构化写作、质量自检和 EPUB 导出串成一套完整流程。

你可以用它处理单本书，也可以把整个书籍文件夹交给它批量处理。Books Skill 会抽取可靠正文，识别扫描版或低质量来源，逐本建立内容地图，输出可学习、可复查、可归档、可二次阅读的 Markdown 与 EPUB。

它面向任何支持自定义技能、系统提示、项目指令或工具脚本的 agent，不绑定特定产品、账号、操作系统或本地目录。**最终产物不是普通 summary，而是接近“替代第一遍精读”的学习材料。**
