...
Books Skill 是一个通用的书籍整理 agent skill。它面向任何支持自定义技能、系统提示、项目指令或工具脚本的 agent，不绑定特定产品、账号、操作系统或本地目录。

它解决的问题很明确：**把单本书或一整个文件夹里的书，整理成可以直接学习、复查、归档和二次阅读的 Markdown 与 EPUB**。它不是“几段摘要”或“目录扩写”，而是一套强调抽取、精读、结构化写作、自检和导出的完整工作流。
...
如果你使用的 agent 具备终端、文件读写和 Git 能力，可以直接把下面这段话发给它，让它自动安装：

```text
请帮我安装 Books Skill：
1. 克隆 https://github.com/PASSIONYOUNGZ/books-skill.git
2. 找到你当前 agent 工具的本地 skills / plugins / custom instructions 目录；如果不确定，就告诉我候选位置并选择最合理的一个。
3. 把仓库里的 books/ 文件夹复制到该目录，保持目录名为 books。
4. 检查 books/SKILL.md 是否存在，并运行 python books/scripts/check_environment.py。
5. 如果你的平台有 skill 校验工具，也请运行校验。
6. 安装完成后告诉我安装路径、依赖状态和如何触发 Books Skill。
```

如果 agent 不知道自己的 skills 目录，可以让它先搜索常见位置，或把 `books/SKILL.md` 内容作为项目指令/系统提示加载。Books Skill 本身不依赖特定平台，只要 agent 能读取说明并执行 Python 脚本，就可以使用。
